...
const Input = React.forwardRef(({ className, type, ...props }, ref) => {
  return (
    <div className="flex items-center w-full rounded-md border border-input bg-background ring-offset-background has-[:focus]:outline has-[:focus]:outline-2">
      {isNotNullOrEmpty(props.icon) && <div className="ml-2">{props.icon}</div>}
      <input
        type={type}
        className={cn(
