...
const Field = ({
  ...
  <InputField>
    <div className={cn("flex divide-x-[1.5px]", className)}>
      <div
        className={cn(
          "min-w-36 flex justify-start py-2 items-center text-nowrap px-4 space-x-2",
          className3
        )}
      >
        {icon && icon}
        {title && <p className="text-gray-700 text-[16px]">{title}</p>}
      </div>
  ...
  <InputField>
    <div className={cn("flex items-center divide-x-[1.5px]", className)}>
      ...
      <div className="flex flex-col items-center justify-start divide-y-[1.5px] w-full">
        {val1 && (
          <div className=" w-full py-1 px-3 text-[16px] text-gray-700 font-semibold">
            {val1}
          </div>
        )}
        {val2 && (
